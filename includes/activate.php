 ...
 * @author Jo Dickson
 */
function activation_error_acf() {
	ob_start();
?>
	<div class="notice notice-error">
		<p><strong>Theme not activated:</strong> The SST WordPress Theme requires the <a href="https://www.advancedcustomfields.com/pro/" target="_blank">Advanced Custom Fields PRO</a> plugin.  Please install and activate ACF PRO and try again.</p>
	</div>
<?php
	echo ob_get_clean();
}
 ...
 */
function activation_checks( $oldtheme_name, $oldtheme ) {
	$do_revert = false;

	// Require ACF PRO
	if ( ! class_exists( 'acf_pro' ) ) {
		$do_revert = true;
		add_action( 'admin_notices', 'activation_error_acf' );
	}
...
add_action( 'after_switch_theme', 'activation_checks', 10, 2 );
